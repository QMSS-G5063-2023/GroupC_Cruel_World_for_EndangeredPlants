...
p <- ggplot() +
  ...
  xlim(-130,-60) +
  geom_point(data = data_cleaned, aes(x = decimalLongitude, y = decimalLatitude, group = year, color = iucn_category), size = 0.25) +
  scale_color_manual(name = "IUCN Category", 
                     values = pal, 
                     labels = paste(levels(data_cleaned$iucn_category), ": ", pal),
                     guide = guide_legend(override.aes = list(size = 5))) +
  coord_fixed() +
  theme_void() +
  transition_time(year) +
  ease_aes('linear') +
  labs(title = "Title of Animated Chart")
...
japanese_zelkova = nytime('\"japanese zelkova\"',2000,2022)
green_ash = nytime('\"green ash\"',2000,2022)
ginkgo_2 = nytime('\"ginkgo \"',2000,2022)
eastern_hemlock = nytime('\"eastern hemlock\"',2000,2022)
american_elm = nytime('\"american elm\"',2000,2022)
